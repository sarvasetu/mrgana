...
_SCAN_ID: ContextVar[str | None] = ContextVar("mrgana_scan_id", default=None)
_AGENT_ID: ContextVar[str | None] = ContextVar("mrgana_agent_id", default=None)
...
class _MrganaContextFilter(logging.Filter):
...
# drowned in their internals when MRGANA_DEBUG=1.
...
_HANDLER_TAG = "_mrgana_scan_handler"


# ``openai.agents`` is the openai-agents SDK's canonical logger root.
_TRACKED_ROOTS: tuple[str, ...] = ("mrgana", "openai.agents")
...
def configure_dependency_logging() -> None:
    """Quiet dependency logging/warnings that obscure Mrgana scan logs."""
...
def setup_scan_logging(run_dir: Path, *, debug: bool | None = None) -> Callable[[], None]:
    ...
    Args:
        run_dir: Per-scan output directory. ``{run_dir}/mrgana.log`` is
        ...
        debug: When ``True``, stderr handler runs at DEBUG instead of
            ERROR. ``None`` (default) reads ``MRGANA_DEBUG`` env: ``1`` /
    ...
    if debug is None:
        debug = (os.environ.get("MRGANA_DEBUG") or "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    log_path = run_dir / "mrgana.log"

    formatter = logging.Formatter(_FORMAT, datefmt=_DATEFMT)
    context_filter = _MrganaContextFilter()
