...
from mrgana.agents.factory import build_mrgana_agent, make_child_factory
from mrgana.agents.prompt import render_system_prompt
...
from mrgana.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from mrgana.core.execution import (
...
from mrgana.core.hooks import BudgetExceededError, ReportUsageHooks
from mrgana.core.inputs import (
    DEFAULT_MAX_TURNS,
    build_root_task,
    build_scope_context,
    make_model_settings,
...
from mrgana.core.sessions import open_agent_session
from mrgana.runtime import session_manager
from mrgana.telemetry.logging import set_scan_id, setup_scan_logging
...
async def run_mrgana_scan(
    *,
    ...
    """Run or resume one Mrgana scan against a sandbox.
    ...
    logger.info(
        "%s Mrgana scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set MRGANA_LLM env or pass model= to run_mrgana_scan().",
        )
    ...
    from mrgana.tools.notes.tools import hydrate_notes_from_disk
    from mrgana.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        logger.warning(
            ...
            "Resume with 'mrgana --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Mrgana scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Mrgana scan %s done", scan_id)
        teardown_logging()
