...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `mrgana/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Mrgana defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `mrgana/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `mrgana.core.sessions.strip_all_images_from_session`, invoked from
  `mrgana/core/execution.py`.
