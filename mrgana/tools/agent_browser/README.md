...
- **Mrgana config:** `containers/Dockerfile` sets `AGENT_BROWSER_*` env
  (executable path, UA, launch args, screenshot dir).
- **Skill:** `mrgana/skills/tooling/agent_browser.md` — **always-loaded**
  into every agent prompt by `mrgana/agents/prompt.py:_resolve_skills`.
