"""SARIF 2.1.0 output for Mrgana vulnerability reports.

Builds a GitHub code-scanning compatible SARIF document from Mrgana findings
...
Design notes:
  * Rules are keyed on CWE (``id = CWE-NNN``), falling back to CVE, then
    ...
    Mrgana output variants (``CWE-306``, ``cwe: 306``, ``306``) to the
    canonical ``CWE-NNN`` form so dedup works across runs.
  * SARIF only has three levels (error / warning / note). Mrgana's five
    severities collapse into them. The raw severity label and CVSS score
    survive in ``result.properties.mrgana`` for downstream tools that can
...
SARIF_VERSION = "2.1.0"
TOOL_NAME = "Mrgana"
TOOL_INFORMATION_URI = "https://mrgana.ai"
...
# SARIF only has three result levels; Mrgana's five severities collapse here.
# Original label survives in ``result.properties.mrgana.severity``.
_SEVERITY_TO_LEVEL = {
    "critical": "error",
    "high": "error",
    "medium": "warning",
...
# mrgana-triage's DEFAULT_STRIDE_LEGS convention).
...
def _apply_repository_context(run: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    ``automationDetails.id`` categorises the run (``mrgana/<owner>/<repo>``)
    so multiple Mrgana runs against the same repo reconcile rather than pile
    ...
    if full_name:
        run["automationDetails"] = {"id": f"mrgana/{full_name}"}
...
def _build_rule(rule_id: str, report: dict[str, Any]) -> dict[str, Any]:
    """Build a SARIF rule descriptor from a Mrgana finding."""
...
def _result_properties(
    ...
    *,
    ...
    """Mrgana-specific metadata for downstream consumers.

    The top-level ``security-severity`` matches GitHub code-scanning's
    expected property. Mrgana-specific fields are namespaced under
    ``mrgana`` so generic SARIF consumers don't see them by default.
    """
    properties: dict[str, Any] = {
        "security-severity": _security_severity(report),
    }
    if class_fingerprint:
        ...
        # filter alerts by it without parsing the nested mrgana.* tree.
        properties["mrgana_vuln_class_hash"] = class_fingerprint
    if is_synthetic:
        # Top-level so reviewers + downstream automation can filter
        # synthetic-anchored alerts without parsing the nested mrgana.*
        # tree.
...
def _build_fixes(report: dict[str, Any]) -> list[dict[str, Any]] | None:
    ...
    Mrgana findings carry the suggested change inline on each code
...
def _rule_id(report: dict[str, Any]) -> str:
    ...
    CWE values are normalised from Mrgana output variants (``CWE-306``,
    ...
    title = _string_value(report.get("title")) or "mrgana-finding"
    ...
    *,
    ...
    Computed from primitives that don't depend on LLM prose stability:
      ...
      - first SARIF location's URI + startLine — these come from
        Mrgana's ``code_locations[].file`` and ``start_line`` which
    ...
    if uri:
        ...
        if start_line is not None:
            ...
            # line because Mrgana code_locations carry the SINK line,
...
def _sarif_level(severity: Any) -> str:
    """Map Mrgana severity labels to SARIF result levels."""
