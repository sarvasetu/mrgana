...
from mrgana.core.paths import run_dir_for, runtime_state_dir
from mrgana.interface.cli import run_cli
from mrgana.interface.tui import run_tui
from mrgana.interface.utils import (
...
from mrgana.report.writer import read_run_record, write_run_record
from mrgana.telemetry import posthog, scarf
from mrgana.telemetry.logging import configure_dependency_logging
...
BEDROCK_EXTRA_HINT = (
    'Bedrock support is optional. Install it with: pipx install "mrgana-agent[bedrock]"'
)
VERTEX_MODEL_MARKER = "vertex"
VERTEX_MISSING_MODULE_ERROR = "No module named 'google"
VERTEX_EXTRA_HINT = (
    'Vertex AI support is optional. Install it with: pipx install "mrgana-agent[vertex]"'
)
...
def validate_environment() -> None:
    ...
    if not settings.llm.model:
        missing_required_vars.append("MRGANA_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "MRGANA_LLM":
                ...
                error_text.append("MRGANA_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "MRGANA_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("MRGANA_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export MRGANA_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_KEY":
                    ...
                    error_text.append(
                        "export MRGANA_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )

        panel = Panel(
            error_text,
            title="[bold white]MRGANA",
            title_align="left",
            border_style="red",
            padding=(1, 2),
        )
...
async def warm_up_llm(show_model_warning: bool = True) -> None:
    ...
    try:
        ...
        if show_model_warning and raw_model and not is_recommended_or_frontier_model(raw_model):
            ...
            warn_text.append(f"'{raw_model}'", style="bold cyan")
            warn_text.append(
                " is not a recommended frontier model for Mrgana.\nSecurity scans work best with:\n",
                style="white",
            )
        ...
        model = MrganaProvider().get_model(raw_model)
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Mrgana Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  mrgana --target https://example.com

  # GitHub repository analysis
  mrgana --target https://github.com/user/repo
  mrgana --target git@github.com:user/repo.git

  # Local code analysis
  mrgana --target ./my-project

  # Large local repository (bind-mounted read-only instead of copied)
  mrgana --mount ./huge-monorepo

  # Domain penetration test
  mrgana --target example.com

  # IP address penetration test
  mrgana --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  mrgana --target https://github.com/user/repo --target https://example.com
  mrgana --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  mrgana --target-list ./targets.txt

  # Custom instructions (inline)
  mrgana --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  mrgana --target example.com --instruction-file ./instructions.txt
  mrgana --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.mrgana/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./mrgana_runs/). "
    ...
    if args.resume:
        ...
        if oversized:
            ...
            parser.error(
                ...
                "(set MRGANA_MAX_LOCAL_COPY_MB to change it). Re-run with "
...
def display_completion_message(args: argparse.Namespace, results_path: Path) -> None:
    ...
    if not scan_completed:
        ...
        resume_text.append("  ")
        resume_text.append(f"mrgana --resume {args.run_name}", style="#22c55e")
        panel_parts.extend(["\n", resume_text])
    ...
    console.print()
    console.print(
        "[#60a5fa]mrgana.ai[/]  [dim]·[/]  "
        "[#60a5fa]docs.mrgana.ai[/]  [dim]·[/]  "
        "[#60a5fa]discord.gg/mrgana-ai[/]"
    )
    console.print()
