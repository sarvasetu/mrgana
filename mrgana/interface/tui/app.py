...
from mrgana.config.models import is_recommended_or_frontier_model
from mrgana.core.hooks import BudgetExceededError
from mrgana.core.runner import run_mrgana_scan
from mrgana.interface.tui.live_view import TuiLiveView
from mrgana.interface.tui.messages import send_user_message_to_agent
from mrgana.interface.tui.renderers import render_tool_widget
from mrgana.interface.tui.renderers.agent_message_renderer import AgentMessageRenderer
from mrgana.interface.tui.renderers.user_message_renderer import UserMessageRenderer
from mrgana.interface.utils import build_tui_stats_text
from mrgana.report.state import ReportState, set_global_report_state
...
def get_package_version() -> str:
    try:
        return pkg_version("mrgana-agent")
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: MrganaTUIApp | None = None

    def set_app_reference(self, app: "MrganaTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _build_url_text(self) -> Text:
        return Text("mrgana.ai", style=Style(color=self.PRIMARY_GREEN, bold=True))

    def _build_welcome_text(self) -> Text:
        text = Text("Welcome to ", style=Style(color="white", bold=True))
        text.append("Mrgana", style=Style(color=self.PRIMARY_GREEN, bold=True))
    ...
    def _build_start_line_text(self, phase: int) -> Text:
        full_text = "Starting Mrgana Agent"
...
class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Mrgana Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class QuitScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Quit Mrgana?", id="quit_title"),
            Grid(
...
class MrganaTUIApp(App):  # type: ignore[misc]
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            MrganaTUIApp._append_renderable(combined, item)
        return MrganaTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(MrganaTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                MrganaTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "content", None) or getattr(item, "renderable", None)
            if inner is not None:
                MrganaTUIApp._append_renderable(combined, inner)
            else:
            ...
            try:
                ...
                try:
                    if not self._scan_stop_event.is_set():
                        image = load_settings().runtime.image or "mrgana-sandbox:latest"
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in MrganaTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in MrganaTUIApp._ICON_PREFIXES:
