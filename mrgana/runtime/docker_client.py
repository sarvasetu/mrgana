"""MrganaDockerSandboxClient — preserves the image's ENTRYPOINT and adds
...
_SANDBOX_NETWORK_ENV = "MRGANA_DOCKER_SANDBOX_NETWORK"
...
def _apply_resource_limits(create_kwargs: dict[str, Any]) -> None:
    ...
    mem_limit = os.environ.get("MRGANA_SANDBOX_MEM_LIMIT", "").strip()
    if mem_limit:
        create_kwargs["mem_limit"] = mem_limit

    shm_size = os.environ.get("MRGANA_SANDBOX_SHM_SIZE", "").strip()
    if shm_size:
        create_kwargs["shm_size"] = shm_size

    cpus = os.environ.get("MRGANA_SANDBOX_CPUS", "").strip()
    if cpus:
    ...
    pids_limit = os.environ.get("MRGANA_SANDBOX_PIDS_LIMIT", "").strip()
    if pids_limit:
        with contextlib.suppress(ValueError):
            create_kwargs["pids_limit"] = int(pids_limit)


class MrganaDockerSandboxSession(DockerSandboxSession):
    sandbox_network: str = ""
...
class MrganaDockerSandboxClient(DockerSandboxClient):
    ...
    mrgana_bind_mounts: list[dict[str, Any]] | None = None
    ...
    async def _create_container(
        ...
        *,
        ...
        # Mrgana delta from the SDK body: drop ``entrypoint`` override and
        ...
        # Mrgana injections — append, don't overwrite, so FUSE/SYS_ADMIN survives.
        ...
        # Mrgana injection: host bind mounts (e.g. large repos passed via --mount)
        # that bypass the SDK's file-by-file LocalDir copy.
        bind_mounts = getattr(self, "mrgana_bind_mounts", ())
        if bind_mounts:
    ...
    async def create(self, **kwargs: Any) -> SandboxSession:
        ...
        if network and isinstance(inner, DockerSandboxSession):
            inner.__class__ = MrganaDockerSandboxSession
            cast("MrganaDockerSandboxSession", inner).sandbox_network = network
        return session
