...
echo -e "${BLUE}🦉 Mrgana Build Script${NC}"
echo "================================"
...
uv run pyinstaller mrgana.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="mrgana-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/mrgana.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/mrgana.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/mrgana" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/mrgana" "$RELEASE_DIR/$BINARY_NAME"
