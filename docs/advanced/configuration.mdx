---
title: "Configuration"
description: "Environment variables for Mrgana"
---

Configure Mrgana using environment variables or a config file.

## LLM Configuration

<ParamField path="MRGANA_LLM" type="string" required>
...
<ParamField path="MRGANA_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="MRGANA_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="MRGANA_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="MRGANA_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Mrgana keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Mrgana still writes complete run telemetry locally to:

```bash
mrgana_runs/<run_name>/events.jsonl
```

When remote vars are set, Mrgana dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="MRGANA_IMAGE" default="ghcr.io/usemrgana/mrgana-sandbox:1.0.0" type="string">
...
<ParamField path="MRGANA_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

<ParamField path="MRGANA_MAX_LOCAL_COPY_MB" default="1024" type="integer">
  Maximum size (in MB) of a local directory target that Mrgana will copy into the sandbox file-by-file. Larger targets exit early with a suggestion to use `--mount` instead. Set to `0` to disable the check.
</ParamField>

## Sandbox Configuration

<ParamField path="MRGANA_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="MRGANA_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Mrgana stores configuration in `~/.mrgana/cli-config.json`. You can also specify a custom config file:

```bash
mrgana --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "MRGANA_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "MRGANA_REASONING_EFFORT": "high"
  }
}
```
...
export MRGANA_SANDBOX_EXECUTION_TIMEOUT="300"
