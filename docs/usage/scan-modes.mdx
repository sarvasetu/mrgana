...
Mrgana offers three scan modes to balance speed and thoroughness.

## Quick

```bash
mrgana --target ./app --scan-mode quick
```
...
```bash
mrgana --target ./app --scan-mode standard
```
...
```bash
mrgana --target ./app --scan-mode deep
```
