---
title: "CLI Reference"
description: "Command-line options for Mrgana"
---

## Basic Usage

```bash
mrgana (--target <target> | --target-list <path> | --mount <path>) [options]
```
...
<ParamField path="--mount" type="string">
  ...
  Mrgana copies local `--target` directories into the sandbox one file at a time, which stalls on very large trees. When a local target exceeds the copy limit (see `MRGANA_MAX_LOCAL_COPY_MB`, default 1024 MB) Mrgana exits early and asks you to re-run with `--mount`.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.mrgana/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget-usd" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, Mrgana enables streaming success callbacks to
...
mrgana --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
mrgana --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
mrgana -n --target ./ --scan-mode quick

# Force diff-scope against a specific base ref
mrgana -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
mrgana -t https://github.com/org/app -t https://staging.example.com
