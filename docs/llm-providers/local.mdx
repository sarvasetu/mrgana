---
title: "Local Models"
description: "Run Mrgana with self-hosted LLMs for privacy and air-gapped testing"
---

Running Mrgana with local models allows for completely offline, privacy-first security assessments. Data never leaves your machine, making this ideal for sensitive internal networks or air-gapped environments.
...
<Warning>
**Compatibility Note**: Mrgana relies on advanced agentic capabilities (tool use, multi-step planning, self-correction). Most local models, especially those under 70B parameters, struggle with these complex tasks.
...
3. Configure Mrgana:
   ```bash
   export MRGANA_LLM="ollama/qwen3-vl"
...
```bash
export MRGANA_LLM="openai/local-model"
